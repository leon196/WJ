...
void main()
{
    ...
    for (int r = 0; r < rayCount; ++r)
    {
        ...
        // Sphere UV
        vec2 uvSphere = vec2(1.0 - mod(atan(p.y, p.x) / PI + 1.0, 1.0), 1.0 - reflectance(p, eye));
        ...
        // Distance to Sphere
        float d = substraction(sphere(eye - originP, 0.5), sphere(p, sphereRadius));
        
        // Distance min or max reached
        if (d < rayEpsilon || t > rayMax)
        {
            // Glow from ray direction
            vec3 c = mix(glowColor, shadowColor, reflectance(originP, eye));
            // Shadow from ray count
            color = mix(color, c, float(r) / float(rayCount));
            // Sky color from distance
            color = mix(color, skyColor, smoothstep(rayMin, rayMax, t) * (1.0 - ratioSky));
            break;
        }
